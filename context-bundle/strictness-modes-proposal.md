...
- **Human Orchestrator (HO):** The human operator. Sets goals, grants approvals, intervenes at impasses, and makes final decisions.
...
Deliberation proceeds through a refinement loop: GA proposes, IA mirrors and counters, GA refines. The loop continues until a handshake or an HO intervention trigger (e.g., an impasse, or an optional max-iteration policy if configured).
...
UG is satisfied when the IA can paraphrase the proposal AND enumerate open verification flags (contradictions, cannot-verify items, and uncertainties) without papering over them. Agreement may include unresolved items that are explicitly logged and accepted by the HO as risks or open questions; `AG_CLOSED` does not imply certainty.

Additional FSM states referenced in this spec:
- **`PLAN_LOCKED`:** The plan has been finalized and locked. No further plan changes without explicitly re-opening AG (HO-controlled) and recording the change in the log.
- **`EXECUTING`:** Implementation is underway based on the locked plan.

Gate closure requires explicit handshake markers in the deliberation log. The HO must approve execution after the Agreement Gate closes.
...
- **Contradiction flagging:** Conflicting information is surfaced to the HO rather than silently resolved by an agent.
...
4. **Separation of concerns.** HO declares intent (manifest fields). Tool computes metrics (log size, tokens, cost). Enforcement is the intersection: declared intent sets floors, computed metrics trigger warnings.
...
7. **One fork point at a time.** Agents may batch logistics (format preferences, delivery method, minor clarifications) in a single message. But when the next HO response would materially change the spec — defaults, floors, enforcement semantics, artifact structure, or gate advancement — isolate that question. The rule prevents accidental commitment at AI speed without creating procedural friction on non-binding exchanges. When in doubt whether something is a fork point, isolate it.
...
### B.1) HO-Declared Fields (Explicit, Human-Authored)
...
deliberation:
  ...
  # --- Strictness fields (all HO-declared) ---
...
|---|---|---|---|---|
| `manifest_status` | enum | `draft` | No | `draft` = auto-generated or not yet reviewed by HO. `ho_qualified` = HO has consciously edited and accepted the manifest. See B.3 for qualification rules. |
| `mode` | enum | `consult` | No | HO's chosen enforcement level. Sets defaults for ceremony requirements. |
| `risk_class` | enum | `normal` + warn | No | HO's assessment of decision criticality. If missing, defaults to `normal` and tool prints: `WARNING: risk_class not declared, defaulting to 'normal'`. In a `draft` manifest, this is an unqualified default — floors do not enforce it. |
...
### B.3) Manifest Qualification (HO Decision: Qualifying Act = Manual Edit)

A manifest exists in one of two states:

| State | How It Gets There | What It Enables |
|---|---|---|
| **`draft`** | Auto-generated by tool, or created by HO but not yet qualified | CONSULT and DELIBERATE only. Floor cascade **disabled**. Tool prints mechanical-metric warnings only. |
| **`ho_qualified`** | HO manually edits the manifest (see qualifying act below) | All modes. Floor cascade **active**. COMMIT and RATIFY unlocked. |
...
| HO qualifies | `### MANIFEST HO-QUALIFIED` — timestamp, type: housekeeping, author: deliberate-cli, detected: [auto-generated] removed + manifest_status=ho_qualified + risk_class explicitly set |
...
| **Handshake artifacts** | Not required | Required for gate closure | Required for gate closure | Required + HO articulation |
...
The tool computes a minimum mode from the manifest's HO-declared fields. This is a pure function — no inference, no heuristics, no model consultation. The cascade evaluates top-to-bottom; first match wins.

**Critical precondition:** The floor cascade is ONLY active when `manifest_status = ho_qualified`. If the manifest is `draft` (including auto-generated stubs), the cascade is bypassed entirely — the tool runs in the HO-selected mode with no floor enforcement, printing only mechanical-metric warnings. This prevents unqualified defaults from triggering binding ceremony requirements.

### D.1) Floor Cascade

```
PRECONDITION: manifest_status = ho_qualified
  (If draft: skip cascade, use HO-selected mode, print WARNING)
...
| Condition | Error |
|---|---|
| Mode is COMMIT/RATIFY and no manifest exists | `ERROR: {MODE} mode requires an HO-qualified manifest. Create and qualify one, or use --mode deliberate.` |
| Mode is COMMIT/RATIFY and `manifest_status` is `draft` | `ERROR: {MODE} mode requires an HO-qualified manifest. Current manifest is draft. Remove [auto-generated] tag, set manifest_status: ho_qualified, and explicitly set risk_class.` |
...
If the HO-selected mode is below the computed floor, the tool refuses unless Ed explicitly acknowledges:
...
# Override acknowledged by HO. Logging to deliberation transcript.
```
...
HO acknowledged running below computed minimum mode.
```
...
- Reads HO-declared manifest fields (explicit tags)
...
```yaml
# Tool-proposed tags (not yet accepted by HO):
proposed_tags:
...
1. Proposed tags are **advisory only** — displayed to HO, never enforced.
2. HO must explicitly accept a proposed tag into the manifest before the floor cascade uses it.
3. Acceptance is recorded as a housekeeping log entry with provenance: `"HO accepted proposed_tag changes_schema=true from semantic layer (turn 7, confidence 0.85)"`.
4. Rejected proposals are logged too: `"HO rejected proposed_tag risk_class=high — reason: exploratory only"`.
...
**HO Decision:** CONSULT is the default mode for new deliberations.
