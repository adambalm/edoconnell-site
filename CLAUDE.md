...
- **Dual editability (2026-07-24):** every content document carries `agentEditable` (default `false`). Agent mutations go ONLY through `scripts/agent-write.mjs` (refuses non-flagged docs; logs each write to `logs/agent-mutations.jsonl`). `artifact` is the node type for made things that are not long-form writing — honest-status vocabulary rendered verbatim, `lastChecked` set only by a real check. `scripts/seed-artifacts.mjs` is dry-run by default; `--execute` is gated on an HO checkpoint plus a fresh `sanity dataset export`.
